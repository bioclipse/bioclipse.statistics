...
/**
 * Rengine class is the interface between an instance of R and the Java VM. Due
 * to the fact that R has no threading support, you can run only one instance of
 * R withing a multi-threaded application. There are two ways to use R from Java:
 * individual call and full event loop. See the Rengine {@link #Rengine
 * constructor} for details.
 * <p>
 * <u>Important note:</u> All methods starting with <code>rni</code> (R Native
 * Interface) are low-level native methods that should be avoided if a
 * high-level methods exists. They do NOT attempt any synchronization, so it is
 * the duty of the calling program to ensure that the invocation is safe (see
 * {@link #getRsync()} for details). At some point in the future when the
 * high-level API is complete they should become private. However, currently
 * this high-level layer is not complete, so they are available for now.
 * <p>
 * All <code>rni</code> methods use <code>long</code> type to reference
 * <code>SEXP</code>s on R side. Those reference should never be modified or
 * used in arithmetics - the only reason for not using an extra interface class
 * to wrap those references is that <code>rni</code> methods are all
 * <i>native</i> methods and therefore it would be too expensive to handle the
 * unwrapping on the C side.
 * <p>
 * <code>jri</code> methods are called internally by R and invoke the
 * corresponding method from the even loop handler. Those methods should usualy
 * not be called directly.
 * <p>
 * Since 0.5 a failure to load the JRI naitve library will not be fatal if
 * <code>jri.ignore.ule=yes</code> system preference is set. Rengine will still
 * not work, but that gives a chance to GUI programs to report the error in a
 * more meaningful way (use {@link #jriLoaded} to check the availability of
 * JRI).
 */
public class Rengine extends Thread {

    /**
     * this flags is set to <code>true</code> if the native code was
     * successfully loaded. If this flag is <code>false</code> then none of the
     * rni methods are available. Previous
     * 
    ...
    static {
        try {
            ...
            if ( iu == null || !iu.equals( "yes" ) ) {
                System.err
                        .println( "Cannot find JRI native library!\nPlease make sure that the JRI native library is in a directory listed in java.library.path.\n" );
                e.printStackTrace();
                // System.exit(1); // why?
            }
        }
    ...
    /**
     * constant to be used in {@link #rniSpecialObject} to return
     * <code>R_NilValue</code> reference
     */
    public static final int SO_NilValue     = 0;
    /**
     * constant to be used in {@link #rniSpecialObject} to return
     * <code>R_GlobalEnv</code> reference
     */
    public static final int SO_GlobalEnv    = 1;
    /**
     * constant to be used in {@link #rniSpecialObject} to return
     * <code>R_EmptyEnv</code> reference
     */
    public static final int SO_EmptyEnv     = 2;
    /**
     * constant to be used in {@link #rniSpecialObject} to return
     * <code>R_baseEnv</code> reference
     */
    public static final int SO_BaseEnv      = 3;
    /**
     * constant to be used in {@link #rniSpecialObject} to return
     * <code>R_UnboundValue</code> reference
     */
    public static final int SO_UnboundValue = 4;
    /**
     * constant to be used in {@link #rniSpecialObject} to return
     * <code>R_MissingArg</code> reference
     */
    public static final int SO_MissingArg   = 5;
    /**
     * constant to be used in {@link #rniSpecialObject} to return
     * <code>R_NaString</code> reference
     */
    public static final int SO_NaString     = 6;
    /**
     * constant to be used in {@link #rniSpecialObject} to return
     * <code>R_BlankString</code> reference
     */
    public static final int SO_BlankString  = 7;

    /**
     * API version of the Rengine itself; see also rniGetVersion() for binary
     * version. It's a good idea for the calling program to check the versions
     * of both and abort if they don't match. This should be done using
     * {@link #versionCheck}
     * 
     * @return version number as <code>long</code> in the form
     *         <code>0xMMmm</code>
     */
    ...
    /**
     * check API version of this class and the native binary. This is usually a
     * good idea to ensure consistency.
     * 
     * @return <code>true</code> if the API version of the Java code and the
     *         native library matches, <code>false</code> otherwise
     */
    ...
    /**
     * debug flag. Set to value &gt;0 to enable debugging messages. The
     * verbosity increases with increasing number
     */
    public static int DEBUG      = 1;

    /**
     * this value specifies the time (in ms) to spend sleeping between checks
     * for R shutdown requests if R event loop is not used. The default is
     * 200ms. Higher values lower the CPU usage but may make R less responsive to
     * shutdown attempts (in theory it should not matter because {@link #stop()}
     * uses interrupt to awake from the idle sleep immediately, but some
     * implementation may not honor that).
     * 
     * @since JRI 0.3
     */
    ...
    /**
     * main engine. Since there can be only one instance of R, this is also the
     * only instance.
     */
    static Rengine    mainEngine = null;

    /**
     * return the current main R engine instance. Since there can be only one
     * true R instance at a time, this is also the only instance. This may not
     * be true for future versions, though.
     * 
     * @return current instance of the R engine or <code>null</code> if no R
     *         engine was started yet.
     */
    ...
    /**
     * returns <code>true</code> if the current thread is the main R thread,
    ...
    public static boolean inMainRThread() {
        ...
        return (mainRThread != null && mainRThread.equals( Thread
                .currentThread() ));
    }
    ...
    /**
     * returns <code>true</code> if this engine was started as a stand-alone
     * Java application or <code>false</code> if this engine was hooked into an
     * existing R instance
     * 
     * @since JRI 0.4
     */
     ...
     * 
     * @param args arguments to be passed to R. Please note that R requires the
     *            presence of certain arguments (e.g. <code>--save</code> or
     *            <code>--no-save</code> or equivalents), so passing an empty
     *            list usually doesn't work.
     * @param runMainLoop if set to <code>true</code> the the event loop will be
     *            started as soon as possible, otherwise no event loop is
     *            started. Running loop requires <code>initialCallbacks</code>
     *            to be set correspondingly as well.
     * @param initialCallbacks an instance implementing the
     *            {@link org.rosuda.JRI.RMainLoopCallbacks RMainLoopCallbacks}
     *            interface that provides methods to be called by R
     */
    ...
    /**
     * create a new engine by hooking into an existing, initialized R instance
     * which is calling this constructor. Currently JRI won't influence this R
     * instance other than disabling stack checks (i.e. no callbacks can be
     * registered etc.). It is *not* the designated constructor and it should be
     * used *only* from withing rJava.
     * 
     * @since JRI 0.4
     */
    public Rengine() {
    ...
    /**
     * RNI: setup R with supplied parameters (should <b>not</b> be used
     * directly!).
     * 
    ...
    /**
     * RNI: setup IPC with RJava. This method is used by rJava to pass the IPC
     * information to the JRI engine for synchronization
     * 
    ...
    /**
     * RNI: lock rJava to allow callbacks - this interrupts R event loop until
     * @link{rniRJavaUnlock} is called.
     * 
     * @return 0 = lock failed, 1 = locked via IPC (you must use rniRJavaUnlock
     *         subsequently), 2 = rJava is already locked
     */
    public native int rniRJavaLock();

    /**
     * RNI: unlock rJava - resumes R event loop. Please note that unlocking
     * without a previously successful lock may cause fatal errors, because it
     * may release a lock issued by another thread which may not have finished
     * yet.
     */
    ...
    /**
     * RNI: parses a string into R expressions (do NOT use directly unless you
     * know exactly what you're doing, where possible use {@link #eval}
    ...
    /**
     * RNI: evaluate R expression (do NOT use directly unless you know exactly
     * what you're doing, where possible use {@link #eval} instead). Note that
     * no synchronization is performed!
     * 
     * @param exp reference to the expression to evaluate
     * @param rho environment to use for evaluation (or 0 for global
     *            environemnt)
     ...
     * @return contents as an array of references or <code>null</code> if the
     *         reference is not VECSXP
     */
     ...
     * 
     * @param exps initial contents of the vector consisiting of an array of
     *            references
     ...
     * @return a list of strings naming all attributes or <code>null</code> if
     *         there are none
     * @since API 1.9, JRI 0.5
     */
     ...
     * @param attr reference to the object to be used as the contents of the
     *            attribute
     */
    public synchronized native void rniSetAttr( long exp, String name, long attr );

    /**
     * RNI: determines whether an R object instance inherits from a specific
     * class (S3 for now)
     * 
     * @since API 1.5, JRI 0.3
     * @param exp reference to an object
     * @param cName name of the class to check
     * @return <code>true</code> if <code>cName</code> inherits from class
     *         <code>cName</code> (see <code>inherits</code> in R)
     */
     ...
     * @param tag TAG
     * @param lang if <code>true</code> then LANGSXP is created, otherwise
     *            LISTSXP.
     * @return reference to the newly created LISTSXP/LANGSXP
     * @since API 1.7, JRI 0.3-7
     */
    public synchronized native long rniCons( long head, long tail, long tag,
                                             boolean lang );
    ...
    public long rniCons( long head, long tail ) {
        ...
        return rniCons( head, tail, 0, false );
    }
    ...
    public long rniLCons( long head, long tail ) {
        ...
        return rniCons( head, tail, 0, true );
    }
    ...
    /**
     * RNI: retrieve CAR part of a dotted-part list recursively as an array of
     * references
     * 
     ...
     * @return name of the symbol or <code>null</code> on error or if exp is no
     *         symbol
     */
     ...
     * RNI: print.
     * <p>
     * <i>Note:</i> May NOT be called inside any WriteConsole callback as it
     * would cause an infinite loop.
     * 
     * @since API 1.8, JRI 0.4
     * @param s string to print (as-is)
     * @param oType output type (see R for exact references, but 0 should be
     *            regular output and 1 error/warning)
     */
    public synchronized native void rniPrint( String s, int oType );

    /**
     * RNI: print the value of a given R object (via print or show method) to
     * the console
     * 
     * @since API 1.9, JRI 0.5
    ...
    /**
     * RNI: preserve object (prevent grabage collection in R) until rniRelease
     * is called.
     * 
     * @since API 1.9, JRI 0.5
     * @param exp reference to an R object
     ...
     * <p>
     * Note: releasing an obejct that was not preserved is an error and results
     ...
     * @param all if set to <code>true</code> then all objects will be shown,
     *            otherwise hidden objects will be omitted
    ...
    /**
     * RNI: return a special object reference. Note that all such references are
     * constants valid for the entire session and cannot be protected/preserved
     * (they are persistent already).
     * 
     * @since API 1.9, JRI 0.5
     * @param which constant referring to a particular special object (see
     *            SO_xxx constants)
     * @return reference to a special object or 0 if the kind of object it
     *         unknown/unsupported
     */
    public synchronized native long rniSpecialObject( int which );

    // --- was API 1.4 but it only caused portability problems, so we got rid of
    // it
    ...
    /**
     * RNI: convert EXTPTRSEXP to Java object - make sure the pointer is really
     * what you expect, otherwise you'll crash the JVM!
     * 
     ...
     *         (usually this means that the binding is locked)
     * @since API 1.10, JRI 0.5-1 (existed before but returned <code>void</code>
     *        )
     */
    public synchronized native boolean rniAssign( String name, long exp,
                                                  long rho );
    ...
    /**
     * RNI: run the main loop.<br>
     * <i>Note:</i> this is an internal method and it doesn't return until the
     * loop exits. Don't use directly!
     */
    ...
    /**
     * Add a handler for R callbacks. The current implementation supports only
     * one handler at a time, so call to this function implicitly removes any
     * previous handlers
     */
    ...
    /**
     * if Rengine was initialized with <code>runMainLoop=false</code> then this
     * method can be used to start the main loop at a later point. It has no
     * effect if the loop is running already. This method returns immediately but
     * the loop will be started once the engine is ready. Please note that there
     * is currently no way of stopping the R thread if the R event loop is
     * running other than using <code>quit</code> command in R which closes the
     * entire application.
     */
    ...
    public void jriWriteConsole( String text, int oType ) {
        ...
        if ( callback != null )
            callback.rWriteConsole( this, text, oType );
    }
    ...
    public void jriBusy( int which ) {
        ...
        if ( callback != null )
            callback.rBusy( this, which );
    }

    /**
     * JRI: R_ReadConsole call-back from R.
     * 
     * @param prompt prompt to display before waiting for the input.<br>
     *            <i>Note:</i> implementations should block for input. Returning
     *            immediately is usually a bad idea, because the loop will be
     *            cycling.
     * @param addToHistory flag specifying whether the entered contents should
     *            be added to history
    ...
    public String jriReadConsole( String prompt, int addToHistory ) {
        ...
        String s =
                (callback == null) ? null : callback
                        .rReadConsole( this, prompt, addToHistory );
        loopHasLock = Rsync.safeLock();
        if ( !loopHasLock ) {
            String es =
                    "\n>>JRI Warning: jriReadConsole detected a possible deadlock ["
                            + Rsync
                            + "]["
                            + Thread.currentThread()
                            + "]. Proceeding without lock, but this is inherently unsafe.\n";
            jriWriteConsole( es, 1 );
            System.err.print( es );
        }
        if ( DEBUG > 1 )
            System.out.println( "Rengine.jreReadConsole END "
                                + Thread.currentThread() );
        return s;
    }
    ...
    public void jriShowMessage( String message ) {
        ...
        if ( callback != null )
            callback.rShowMessage( this, message );
    }
    ...
    public void jriLoadHistory( String filename ) {
        ...
        if ( callback != null )
            callback.rLoadHistory( this, filename );
    }
    ...
    public void jriSaveHistory( String filename ) {
        ...
        if ( callback != null )
            callback.rSaveHistory( this, filename );
    }

    /**
     * JRI: R_ChooseFile call-back from R
     * 
     * @param newFile flag specifying whether an existing or new file is
     *            requested
    ...
    public String jriChooseFile( int newFile ) {
        ...
        if ( callback != null )
            return callback.rChooseFile( this, newFile );
        return null;
    }
    ...
    public void jriFlushConsole() {
        ...
        if ( callback != null )
            callback.rFlushConsole( this );
    }

    // ============ "official" API =============

    /**
     * Parses and evaluates an R expression and returns the result. Has the same
     * effect as calling <code>eval(s, true)</code>.
     * 
     ...
     * @since JRI 0.3
     * @param s expression (as string) to parse and evaluate
     * @param convert if set to <code>true</code> the resulting REXP will
     *            contain native representation of the contents, otherwise an
     *            empty REXP will be returned. Depending on the back-end an
     *            empty REXP may or may not be used to convert the result at a
     *            later point.
     * @return resulting expression or <code>null</code> if something wnet wrong
     */
    public synchronized REXP eval( String s, boolean convert ) {
        ...
        if ( DEBUG > 0 )
            System.out.println( "Rengine.eval(" + s + "): BEGIN "
                                + Thread.currentThread() );
        boolean obtainedLock = Rsync.safeLock();
        try {
            /*
             * --- so far, we ignore this, because it can happen when a callback
             * needs an eval which is ok ... if (!obtainedLock) { String
             * es="\n>>JRI Warning: eval(\""
             * +s+"\") detected a possible deadlock ["
             * +Rsync+"]["+Thread.currentThread
             * ()+"]. Proceeding without lock, but this is inherently unsafe.\n"
             * ; jriWriteConsole(es); System.err.print(es); }
             */
            ...
            if ( pr != 0 ) {
                ...
                if ( er != 0 ) {
                    ...
                    if ( DEBUG > 0 )
                        System.out.println( "Rengine.eval(" + s + "): END (OK)"
                                            + Thread.currentThread() );
                    return x;
                }
            }
        ...
        if ( DEBUG > 0 )
            System.out.println( "Rengine.eval(" + s + "): END (ERR)"
                                + Thread.currentThread() );
        return null;
    }

    /**
     * This method is very much like {@link #eval(String)}, except that it is
     * non-blocking and returns <code>null</code> if the engine is busy.
     * 
     * @param s string to evaluate
     * @return result of the evaluation or <code>null</code> if the engine is
     *         busy
     */
    ...
    /**
     * This method is very much like {@link #eval(String,boolean)}, except that
     * it is non-blocking and returns <code>null</code> if the engine is busy.
     * 
     * @since JRI 0.3
     * @param s string to evaluate
     * @param convert flag denoting whether an empty or fully-converted REXP
     *            should be returned (see {@link #eval(String,boolean)} for
     *            details)
     * @return result of the evaluation or <code>null</code> if the engine is
     *         busy
     */
    public synchronized REXP idleEval( String s, boolean convert ) {
        ...
        if ( lockStatus == 1 )
            return null; // 1=locked by someone else
    ...
    /**
     * returns the synchronization mutex for this engine. If an external code
     * needs to use RNI calls, it should do so only in properly protected
     * environment secured by this mutex. Usually the procedure should be as
     * follows:
     * 
     * <pre>
     ...
     * 
     * @return <code>true</code> if R is alive and <code>false</code> if R died
     *         or exitted
     */
    ...
    /**
     * The implementation of the R thread. This method should not be called
     * directly.
     */
    public void run() {
        ...
        loopHasLock = Rsync.safeLock(); // force all code to wait until R is
                                        // ready
        try {
            if ( setupR( args ) == 0 ) {
                if ( !runLoop && loopHasLock ) { // without event loop we can
                                                 // unlock now since we woin't
                                                 // do anything
                    Rsync.unlock();
                    loopHasLock = false;
                }
    ...
    /**
     * assign a string value to a symbol in R. The symbol is created if it
     * doesn't exist already.
     * 
     * @param sym symbol name. The symbol name is used as-is, i.e. as if it was
     *            quoted in R code (for example assigning to "foo$bar" has the
     *            same effect as `foo$bar`&lt;- and NOT foo$bar&lt;-).
     * @param ct contents
    ...
    /**
     * assign a content of a REXP to a symbol in R. The symbol is created if it
     * doesn't exist already.
     * 
     * @param sym symbol name. The symbol name is used as-is, i.e. as if it was
     *            quoted in R code (for example assigning to "foo$bar" has the
     *            same effect as `foo$bar`&lt;- and NOT foo$bar&lt;-).
     * @param r contents as <code>REXP</code>. currently only raw references and
     *            basic types (int, double, int[], double[], boolean[]) are
     *            supported.
     * @return <code>true</code> if successful, <code>false</code> otherwise
     *         (usually locked binding or unsupported REXP)
     * @since JRI 0.3 (return value changed to boolean in JRI 0.5-1)
     */
    public boolean assign( String sym, REXP r ) {
        ...
        try {
            ...
            if ( r.Xt == REXP.XT_INT || r.Xt == REXP.XT_ARRAY_INT ) {
                int[] cont =
                        r.rtype == REXP.XT_INT ? new int[] { ((Integer) r.cont)
                                .intValue() } : (int[]) r.cont;
            ...
            if ( r.Xt == REXP.XT_DOUBLE || r.Xt == REXP.XT_ARRAY_DOUBLE ) {
                double[] cont =
                        r.rtype == REXP.XT_DOUBLE ? new double[] { ((Double) r.cont)
                                .intValue() } : (double[]) r.cont;
            ...
            if ( r.Xt == REXP.XT_STR || r.Xt == REXP.XT_ARRAY_STR ) {
                String[] cont =
                        r.rtype == REXP.XT_STR ? new String[] { (String) r.cont }
                                : (String[]) r.cont;
    ...
    /**
     * assign values of an array of doubles to a symbol in R (creating an
     * integer vector).<br>
    ...
    /**
     * assign values of an array of integers to a symbol in R (creating a
     * numeric vector).<br>
     * equals to calling {@link #assign(String, REXP)}.
     * 
     * @param sym symbol name
    ...
    /**
     * assign values of an array of booleans to a symbol in R (creating a
     * logical vector).<br>
     * equals to calling {@link #assign(String, REXP)}.
     * 
     * @param sym symbol name
    ...
    /**
     * assign values of an array of strings to a symbol in R (creating a
     * character vector).<br>
     * equals to calling {@link #assign(String, REXP)}.
     * 
     * @param sym symbol name
    ...
    /**
     * creates a <code>jobjRef</code> reference in R via rJava.<br>
     * <b>Important:</b> rJava must be loaded and intialized in R (e.g. via
     * <code>eval("{library(rJava);.jinit()}",false)</code>, otherwise this will
     * fail. Requires rJava 0.4-13 or higher!
     * 
     * @param o object to push
     * @return Pure REXP reference of the newly created <code>jobjRef</code>
     *         object or <code>null</code> upon failure. It will have the type
     *         <code>XT_NONE</code> such that it can be used in
     *         @link{assign(String, REXP)}.
     * @since JRI 0.3-7
     */
    public REXP createRJavaRef( Object o ) {
        ...
        try {
            long l =
                    rniEval( rniLCons( rniInstallSymbol( ".jmkref" ),
                                       rniLCons( rniJavaToXref( o ),
                                                 rniLCons( rniPutString( klass ),
                                                           0 ) ) ), 0 );
            if ( l <= 0 && l > -4 )
                return null; /*
                              * for safety failure codes are only -3 .. 0 to not
                              * clash with 64-bit pointers
                              */
